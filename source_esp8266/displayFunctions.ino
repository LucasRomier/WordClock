...
void adjustBrightness(int hour) {
  ...
  if (_MORNINGCUTOFF < _NIGHTCUTOFF) {
      if(hour>=_MORNINGCUTOFF && hour<_NIGHTCUTOFF) {
        FastLED.setBrightness(_DAYBRIGHTNESS); //day time
        return;
        }
  } else {
      if(hour>=_MORNINGCUTOFF || hour<=_NIGHTCUTOFF) {
  ...
  FastLED.setBrightness(_NIGHTBRIGHTNESS);
}
